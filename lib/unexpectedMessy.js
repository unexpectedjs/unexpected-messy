                ...
                promiseByKey = {
                    headersPresent: expect.promise(function () {
                        if (value && typeof value === 'object' && !value.isMessyHeaders) {
                            Object.keys(value).forEach(function (headerName) {
                                if (typeof value[headerName] === 'undefined') {
            ...
            subject.getNames().forEach(function (headerName) {
                ...
                promiseByKey.headers[headerName] = expect.promise.all(
                    satisfySpecs.map(function (satisfySpec, i) {
                        var promisesForThisSatisfySpec = Object.keys(promisesByHeaderValue).map(function (headerValue) {
                            return promisesByHeaderValue[headerValue][i];
                        });
                        return expect.promise.any(promisesForThisSatisfySpec);
                    })
                );
            });
            ...
            return expect.promise.all(promiseByKey).caught(function (err) {
                ...
                return expect.promise.settle(promiseByKey).then(function (promises) {
                    promises.forEach(function (promise) {
                        var reason = promise.isRejected() && promise.reason();
                        if (reason && !reason._isUnexpected && reason.message !== 'aggregate error') {
                            expect.fail(promise.reason());
                        }
                    });
                        ...
                        output.block(function () {
                            ...
                            subject.getNames().forEach(function (headerName) {
                                ...
                                if (subjectHeaderValues) {
                                    for (i = 0 ; i < subjectHeaderValues.length ; i += 1) {
                                        subjectHeaderValue = subjectHeaderValues[i];subjectHeaderValue
                                        outputHeader(headerName, subjectHeaderValue);
                                        if (remainingExpectedHeaders.has(headerName)) {
