  ...
  installInto: function unexpectedMessy(expect) {
    ...
    expect.exportType({
      ...
      diff: function(actual, expected, output, diff, inspect, equal) {
        output.block(function(output) {
          ...
          var remainingActualHeaders = actual.clone();
            ...
            if (actualHeaderValues) {
              for (i = 0; i < actualHeaderValues.length; i += 1) {
                ...
                if (remainingExpectedHeaders.has(headerName)) {
                  var expectedHeaderValue = remainingExpectedHeaders.get(
                    headerName
                  );
                  output
    ...
    expect.exportAssertion(
      ...
      function(expect, subject, value) {
        ...
        var promisesByHeaderNameAndValue = {};
        subject.getNames().forEach(function(headerName) {
          ...
          var promisesByHeaderValue = (promisesByHeaderNameAndValue[
            headerName
          ] = {});
            ...
            expect.fail({
              diff: function(output, diff, inspect, equal) {
                ...
                output.block(function(output) {
                  ...
                  var remainingExpectedHeaders =
                    value && value.isMessyHeaders
                      ? value.clone()
                      : new messy.Headers(value, true);

                  var remainingSubjectHeaders = subject.clone();

                  subject.getNames().forEach(function(headerName) {
                    // First pass: Find exact matches
                    var subjectHeaderValues = subject.getAll(headerName);

                    var subjectHeaderValue;
      ...
      function(expect, subject, value) {
        ...
        if (value && typeof value === 'object' && !value.isMessyMessage) {
          ...
          if (unsupportedKeys.length > 0) {
            throw new Error(
              `messy.Message to satisfy: Unsupported keys: ${ 
              ...
              diff: function(output, diff, inspect, equal) {
                output.block(function(output) {
                  if (promiseByKey.headers.isRejected()) {
                    var comparison = promiseByKey.headers
                      .reason()
                      .getDiff(output);
                    if (comparison) {
                  ...
                  if (
                    ...
                    output.nl().annotationBlock(function() {
                      ...
                      var unchunkedBodyDiff = unchunkedBodyError.getDiff(
                        output
                      );
                  ...
                  if (typeof value.parts !== 'undefined') {
                    ...
                    if (Array.isArray(value.parts)) {
                      ...
                      var nonNumericPartIds = Object.keys(value.parts).filter(
                        function(partId) {
    ...
    expect.exportType({
      ...
      diff: function(actual, expected, output, diff, inspect, equal) {
        ...
        output.block(function() {
          ...
          var actualMetadata = _.pick(
            actual,
            messy.HttpRequest.metadataPropertyNames
          );

          var expectedMetadata = _.pick(
            expected,
            messy.HttpRequest.metadataPropertyNames
          );
              ...
              diff: function(output, diff, inspect, equal) {
                ...
                var error = output.clone().annotationBlock(function() {
                  if (
                    ...
                    messy.StatusLine.propertyNames.some(function(propertyName) {
                      return (
          ...
          Object.keys(value.exchanges).forEach(function(key, i) {
            promiseByKey[key] = expect.promise(function() {
              if (i < subject.exchanges.length) {
                ...
                var instanceAndSatisfySpec = convertSatisfySpecToInstance(
                  value.exchanges[key],
                  messy.HttpExchange
                );
                return expect(
