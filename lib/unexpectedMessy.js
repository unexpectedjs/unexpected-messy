...
function getUpgradedBody(message) {
    ...
    if (typeof body !== 'undefined') {
        ...
        if ((typeof Buffer === 'function' && Buffer.isBuffer(body)) && isTextualContentType(message.headers.get('Content-Type')) && bufferCanBeInterpretedAsUtf8(body)) {
...
function upgradeOrDowngradeMessageBodyToMatchSpecBody(message, spec) {
    ...
    if (typeof messageBody !== 'undefined') {
        ...
        if (typeof Buffer === 'function' && Buffer.isBuffer(messageBody) && ((typeof specBody === 'string' || isRegExp(specBody) || isNonBufferNonRegExpObject(specBody)) || (bufferCanBeInterpretedAsUtf8(messageBody) && isTextualContentType(message.headers.get('Content-Type'))))) {
            try {
    ...
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            if (!actual.hasEmptyBody() || !expected.hasEmptyBody()) {
                ...
                var bodyDiff = diff(actualBody, expectedBody);
                if (bodyDiff) {
