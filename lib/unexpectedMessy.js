...
module.exports = function unexpectedMessy(expect) {
    expect.addType({
        ...
        diff: function (output, actual, expected, diff, inspect, equal) {
            ...
            actual.upgradeOrDowngradeBodyToMatchSpec(expected.body);
    ...
    .addType({
        ...
        inspect: function (output, httpRequest, inspect) {
            return output.append(inspect(httpRequest.requestLine))
                .nl()
                .append(this.baseType.inspect(output, httpRequest, inspect)); //MJMMMMMMMMMMM
        },
        diff: function (output, actual, expected, diff, inspect, equal) {
            output
        ...
        inspect: function (output, httpResponse, inspect) {
            return output.append(inspect(httpResponse.statusLine))
                .nl()
                .append(this.baseType.inspect(output, httpResponse, inspect)); // HMMMMMMMMMMMM
        },
        diff: function (output, actual, expected, diff, inspect, equal) {
            output
        ...
        inspect: function (output, httpExchange, inspect) {
            return output.append(inspect(httpExchange.request))
                .nl(2)
                .append(httpExchange.response);
        },
        diff: function (output, actual, expected, diff, inspect, equal) {
            output
        ...
        diff: function (output, actual, expected, diff, inspect, equal) {
            for (var i = 0 ; i < Math.max(actual.exchanges.length, expected.exchanges.length) ; i += 1) {
                if (i < actual.exchanges.length && i < expected.exchanges.length) {
                    ...
                    output
                        .append(diff(actual.exchanges[i], expected.exchanges[i]).diff);
                } else {
                    throw new Error('Not implemented: diffing conversations with a different number of exchanges');
                }
            }
