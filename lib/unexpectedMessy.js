    ...
    installInto: function unexpectedMessy(expect) {
        ...
        .addAssertion('messy.Headers', 'to [exhaustively] satisfy', function (expect, subject, value) {
            ...
            var exhaustively = this.flags.exhaustively,
                ...
                satisfySpecPromisesByHeaderNameAndValue = {};
            subject.getNames().forEach(function (headerName) {
                ...
                var promisesByHeaderValue = satisfySpecPromisesByHeaderNameAndValue[headerName] = {};
                ...
                return expect.promise.settle(promiseByKey).then(function (promises) {
                    expect.fail({
                        diff: function (output, diff, inspect, equal) {
                            ...
                            Object.keys(satisfySpecPromisesByHeaderNameAndValue).forEach(function (headerName) {
                                remainingSatisfySpecPromisesByHeaderNameAndValue[headerName] = {};
                                Object.keys(satisfySpecPromisesByHeaderNameAndValue[headerName]).forEach(function (headerValue) {
                                    remainingSatisfySpecPromisesByHeaderNameAndValue[headerName][headerValue] = [].concat(satisfySpecPromisesByHeaderNameAndValue[headerName][headerValue]);
                                });
                            });
                            output.block(function () {
                                var output = this; // Sorry for shadowing
                        ...
                        diff: function (output, diff, inspect, equal) {
                            output.block(function () {
                                ...
                                if (bodyDiffResult && bodyDiffResult.diff) {
                                    ...
                                    if (bodySatisfyError) {
                                        output.sp().error('// ' + (bodySatisfyError.label || 'should satisfy') + ' ').append(inspect(value.body));
                                    }
                                } else if (bodySatisfyError) {
                                    output.nl(2).error('// ' + (bodySatisfyError.label || 'should satisfy') + ' ').append(inspect(value.body));
                                ...
                                if (typeof value.parts !== 'undefined') {
                                    ...
                                    if (Array.isArray(value.parts)) {
                                        ...
                                        if (promiseByKey.parts.isRejected()) {
                                            ...
                                            var d2 = partsError.createDiff && partsError.createDiff(output.clone(), diff, inspect, equal);
                                            output.nl().annotationBlock(function () {
                                                this.append((d2 && d2.diff) || partsError.output);
                                            });
                                        }
                                    } else if (value.parts && typeof value.parts === 'object') {
