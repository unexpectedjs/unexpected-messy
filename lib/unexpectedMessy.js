    ...
    try {
        if (value && typeof value === 'object' && !value.isMessyMessage) {
            var unsupportedKeys = Object.keys(value).filter(function (key) {
                return key !== 'body' && key !== 'fileName' && key !== 'decodedBody' && key !== 'rawSrc' && key !== 'text' && key !== 'headers' && key !== 'parts';
            });
            ...
            if ('rawSrc' in value) {
                expect(subject, 'to have rawSrc satisfying', value.rawSrc);
            }
            if ('text' in value) {
                ...
                output.block(function () {
                    ...
                    if ('fileName' in value) {
                        try {
                            ...
                            if (e3._isUnexpected) {
                                ...
                                var d = e3.createDiff && e3.createDiff(output.clone(), diff, inspect, equal);
                                if (d && d.diff) {
                        ...
                        try {
                            ...
                            if (e4._isUnexpected) {
                                output.nl().annotationBlock(function () {
                                    ...
                                    var d = e4.createDiff && e4.createDiff(output.clone(), diff, inspect, equal);
                                    if (d && d.diff) {
                        ...
                        try {
                            ...
                            if (e5._isUnexpected) {
                                output.nl().annotationBlock(function () {
                                    ...
                                    var d = e5.createDiff && e5.createDiff(output.clone(), diff, inspect, equal);
                                    if (d && d.diff) {
                                        output.nl().append(d.diff);
                                    }
                                });
                    ...
                    if ('rawSrc' in value) {
                        try {
                            ...
                            if (e6._isUnexpected) {
                                output.nl().annotationBlock(function () {
                                    this.error('should have rawSrc satisfying').sp().append(expect.inspect(value.rawSrc));
                                    var d = e6.createDiff && e6.createDiff(output.clone(), diff, inspect, equal);
                                    if (d && d.diff) {
                                        this.nl().append(d.diff);
                                    }
                                });
...
module.exports = {
    ...
    installInto: function unexpectedMessy(expect) {
                                        ...
                                        if (remainingExpectedHeaders.has(headerName)) {
                                            ...
                                            if (typeof expectedHeaderValue === 'undefined') {
                                                ...
                                                try {
                                                    ...
                                                    output.sp(subjectHeaderValue === '' ? 0 : 1).error('// ' + (e.label || 'should satisfy') + ' ').messyHeaderValue(expectedHeaderValue);
                                                }
                                                remainingExpectedHeaders.remove(headerName, 0);
                                            }
                                        } else if (exhaustively) {
        ...
        .addAssertion('messy.Message', 'to have rawSrc satisfying', function (expect, subject, value) {
            expect(subject.rawSrc, 'to satisfy', value);
        })
        ...
        .addType({
            ...
            try {
                ...
                expect(_.pick(subject, messy.HttpRequest.metadataPropertyNames), 'to satisfy', _.pick(value, messy.HttpRequest.metadataPropertyNames));
                        ...
                        output.block(function () {
                            ...
                            messy.HttpRequest.metadataPropertyNames.forEach(function (metadataPropertyName) {
                                if (metadataPropertyName in value) {
                                    try {
                                        topLevelExpect(subject[metadataPropertyName], 'to satisfy', value[metadataPropertyName]);
                                    } catch (e2) {
                                        if (e2._isUnexpected) {
                                            if (metadataPropertyName === 'encrypted' && value.encrypted === true) {
                                                ...
                                                output.nl().annotationBlock(function () {
                                                    this.text(metadataPropertyName).text(':').sp().append(e2.output);
                                                    var d = e2.createDiff && e2.createDiff(output.clone(), diff, inspect, equal);
                                                    if (d && d.diff) {
                                                        this.nl().sp(2).block(d.diff);
                                                    }
                                                });
                                            }
                                        } else {
            ...
            try {
                if (typeof value.exchanges !== 'undefined') {
                    ...
                    if (typeof value.exchanges === 'function') {
                        expect(subject.exchanges, 'to satisfy', value.exchanges);
                ...
                if (!value.isMessyConversation && typeof value.exchanges !== 'undefined' && typeof value.exchanges !== 'function') {
                    if (Array.isArray(value.exchanges)) {
                        ...
                        Object.keys(value.exchanges).forEach(function (key) {
                            expect(subject.exchanges[key], 'to [exhaustively] satisfy', value.exchanges[key]);
                        });
                    }
                }
            } catch (e) {
                if (e._isUnexpected) {
                    e.createDiff = function (output, diff, inspect, equal) {
                        function inspectSatisfySpecAsInstance(obj, Constructor) {
                            var satisfySpec = {},
                                instance;
                            ...
                            try {
                                expect(instance, 'to satisfy', satisfySpec);
                            } catch (e) {
                                return e.createDiff(output.clone(), diff, inspect, equal).diff;
                            }
                        ...
                        output.block(function () {
                            ...
                            for (var i = 0 ; i < Math.max(subject.exchanges.length, numValueExchanges) ; i += 1) {
                                ...
                                if (i < subject.exchanges.length && i < numValueExchanges) {
                                    var exchangeSatisfyError,
                                        exchangeSatisfyDiff;
                                    if (typeof value.exchanges !== 'function' && i in value.exchanges) {
                                        try {
                                            expect(subject.exchanges[i], 'to [exhaustively] satisfy', value.exchanges[i]);
                                        } catch (_exchangeSatisfyError) {
                                            exchangeSatisfyError = _exchangeSatisfyError;
                                            exchangeSatisfyDiff = exchangeSatisfyError.createDiff && exchangeSatisfyError.createDiff(output.clone(), diff, inspect, equal);
                                        }
                                    ...
                                    output.annotationBlock(function () {
                                        this.error('missing:').nl().append(inspectSatisfySpecAsInstance(value.exchanges[i], messy.HttpExchange));
                                    });
                                } else {
                                    output.annotationBlock(function () {
