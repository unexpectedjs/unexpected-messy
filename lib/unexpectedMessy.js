...
module.exports = function unexpectedMessy(expect) {
    expect.addType({
        ...
        inspect: function (output, message, inspect, depth) {
    ...
    .addType({
        ...
        inspect: function (output, httpRequest, inspect, depth) {
            return (output
                .append(inspect(httpRequest.requestLine, depth))
                .nl(httpRequest.headers.getNames().length > 0 ? 2 : 0)
                .append(inspect(expect.cast('messy.Message', httpRequest), depth))
            );
        },
        diff: function (output, actual, expected, diff, inspect, equal) {
            return {
                diff: output
                    ...
                    .nl()
                    .append(diff(expect.cast('messy.Message', actual), expect.cast('messy.Message', expected)).diff)
            };
        }
    }).addAssertion('messy.HttpRequest', 'to [exhaustively] satisfy', function (expect, subject, value) {
        expect(expect.cast('messy.Message', subject), 'to [exhaustively] satisfy', value);
        ...
        inspect: function (output, httpResponse, inspect, depth) {
            return (output
                .append(inspect(httpResponse.statusLine, depth))
                .nl(httpResponse.headers.getNames().length > 0 ? 2 : 0)
                .append(inspect(expect.cast('messy.Message', httpResponse), depth))
            );
        },
        diff: function (output, actual, expected, diff, inspect, equal) {
            return {
        ...
        inspect: function (output, httpExchange, inspect, depth) {
        ...
        inspect: function (output, httpConversation, inspect, depth) {
