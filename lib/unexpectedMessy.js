...
module.exports = {
    ...
    installInto: function unexpectedMessy(expect) {
        var parentExpect = expect;
        expect = expect.child();

        var topLevelExpect = expect;

        parentExpect.use(require('magicpen-media'));

        // The below styles must be added to the parent because some of the
        // types added to the parent have inspect methods that depend on them:
        parentExpect.addStyle('messyHeaderValue', function (value) {
        ...
        parentExpect.addStyle('messyHeader', function (name, value) {
            this.text(messy.formatHeaderName(name) + ':', 'gray')
                .text(' ').messyHeaderValue(value);
        });

        parentExpect.addStyle('messyMessageBody', function (message, depth) {
