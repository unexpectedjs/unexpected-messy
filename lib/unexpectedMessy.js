...
module.exports = {
    ...
    installInto: function unexpectedMessy(expect) {
        ...
        .addAssertion('messyHeaders', 'to [exhaustively] satisfy', function (expect, subject, value) {
            if (isRegExp(value)) {
                return expect(subject.toString(), 'to match', value);
            }
            var exhaustively = this.flags.exhaustively,
                expectedHeaders = value && value.isMessyHeaders ? value : new messy.Headers(value, true),
                promiseByKey = {
                    ...
                    if (!actual.hasEmptyBody() || !expected.hasEmptyBody()) {
                        ...
                        if (bodyDiff) {
                            ...
                            if (typeof actual.body === 'string') {
                                output.code(actual.body, sanitizeContentType(actual.headers.get('Content-Type')));
                            } else {
                                // Buffer instance or parsed JSON
        ...
        .addAssertion('messyMessage', 'to [exhaustively] satisfy', function (expect, subject, value) {
                        ...
                        diff: function (output, diff, inspect, equal) {
                            output.block(function (output) {
                                if (promiseByKey.headers.isRejected()) {
                                    var comparison = promiseByKey.headers.reason().getDiff();
                                    if (comparison) {
                                ...
                                var bodySatisfyError = promiseByKey.body.isRejected() && promiseByKey.body.reason(),
                                    bodyDiffResult = bodySatisfyError && bodySatisfyError.getDiff();
                                ...
                                if (bodyDiffResult) {
                                    ...
                                    if (subject.isMultipart) {
                                        subject.parts.forEach(function (part, i) {
                                            ...
                                            var partSatisfyError = promiseByKey[i] && promiseByKey[i].isRejected() && promiseByKey[i].reason(),
                                                partSatisfyDiff = partSatisfyError && partSatisfyError.getDiff();
                                    ...
                                    } else if (typeof subject.body === 'string') {
                                        output.code(subject.body, sanitizeContentType(subject.headers.get('Content-Type')));
                                    } else {
                                        // Buffer instance or parsed JSON
                                ...
                                if (promiseByKey.unchunkedBody && promiseByKey.unchunkedBody.isRejected()) {
                                    ...
                                    output.nl().annotationBlock(function () {
                                        ...
                                        var d = unchunkedBodyError.getDiff();
                                        if (d) {
                                            this.nl().append(d.diff);
                                        }
                                    });
                                ...
                                if (typeof value.parts !== 'undefined') {
                                    ...
                                    if (Array.isArray(value.parts)) {
                                        ...
                                        if (promiseByKey.parts.isRejected()) {
                                            ...
                                            output.nl().annotationBlock(partsError.getDiffMessage());
                                        }
                                    } else if (value.parts && typeof value.parts === 'object') {
        ...
        .addAssertion('messyMessage', 'to have raw body satisfying', function (expect, subject, value) {
            expect(subject.rawBody, 'to satisfy', value);
        })
        .addAssertion('messyMessage', 'to have unchunked body satisfying', function (expect, subject, value) {
            expect(subject.unchunkedBody, 'to satisfy', value);
        })
        .addAssertion('messyMessage', 'to have file name satisfying', function (expect, subject, value) {
            expect(subject.fileName, 'to satisfy', value);
        })
        .addAssertion('messyMessage', 'to have text satisfying', function (expect, subject, value) {
            expect(subject.toString(), 'to satisfy', value);
        })
        .addAssertion('messyMessage', '[not] to be a multipart message', function (expect, subject) {
            expect(subject.isMultipart, '[not] to be truthy');
        })
        .addAssertion('messyMessage', '[not] to have number of parts', function (expect, subject, value) {
        ...
        .addAssertion('messyRequestLine', 'to [exhaustively] satisfy', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string') {
                value = omitUndefinedProperties(_.pick(new messy.RequestLine(value), messy.RequestLine.propertyNames));
            }
            var flags = this.flags;
            return expect.promise(function () {
                if (value && typeof value === 'object') {
            ...
            diff: function (actual, expected, output, diff, inspect, equal) {
                ...
                output.block(function () {
                    ...
                    if (!equal(actualMetadata, expectedMetadata)) {
                        this.nl().text('Metadata:').sp().append(diff(actualMetadata, expectedMetadata).diff);
                    }
                });
                return { inline: true, diff: output };
            }
        })
        .addAssertion('messyHttpRequest', 'to [exhaustively] satisfy', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string' || (typeof Buffer === 'function' && Buffer.isBuffer(value))) {
                value = omitUndefinedProperties(_.pick(new messy.HttpRequest(value), messy.HttpRequest.propertyNames));
            }
                        ...
                        diff: function (output, diff, inspect, equal) {
                            output.block(function (output) {
                                ...
                                if (promiseByKey.requestLine.isRejected()) {
                                    ...
                                    diffResult = requestLineError.getDiff();
                                    if (diffResult) {
                                        output.append(diffResult.diff);
                                    } else {
                                        output.annotationBlock(requestLineError.getErrorMessage());
                                    }
                                } else if (promiseByKey.requestLineSpec.isRejected()) {
                                    var requestLineSpecError = promiseByKey.requestLineSpec.reason();
                                    diffResult = requestLineSpecError.getDiff();
                                    if (diffResult) {
                                        output.append(diffResult.diff);
                                    } else {
                                        output.annotationBlock(requestLineSpecError.getErrorMessage());
                                    }
                                } else {
                                ...
                                if (promiseByKey.message.isRejected()) {
                                    ...
                                    diffResult = messageError.getDiff();
                                    if (diffResult) {
                                        output.append(diffResult.diff);
                                    } else {
                                        output.annotationBlock(messageError.getErrorMessage());
                                    }
                                } else {
                                ...
                                messy.HttpRequest.metadataPropertyNames.forEach(function (metadataPropertyName) {
                                    ...
                                    if (metadataPropertyError) {
                                        if (metadataPropertyName === 'encrypted' && value.encrypted === true) {
                                            ...
                                            output.nl().annotationBlock(function () {
                                                this.text(metadataPropertyName).text(':').sp().append(metadataPropertyError.getErrorMessage());
                                            });
                                        }
                                    }
                                });
        ...
        .addAssertion('messyStatusLine', 'to [exhaustively] satisfy', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string') {
                value = omitUndefinedProperties(_.pick(new messy.StatusLine(value), messy.StatusLine.propertyNames));
            }
            var flags = this.flags;
                    ...
                    diff: function (output, diff, inspect, equal) {
                        ...
                        if (typeof value === 'number' || typeof value === 'function') {
                            if (value._expectIt) {
                                ...
                                if (comparison) {
                                    output.annotationBlock(comparison.diff);
                                }
                            } else {
        ...
        .addType({
            ...
            diff: function (actual, expected, output, diff, inspect, equal) {
                var baseType = this.baseType;
                output.block(function (output) {
                    output.append(diff(actual.statusLine, expected.statusLine).diff);
        ...
        .addAssertion('messyHttpResponse', 'to [exhaustively] satisfy', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string' || (typeof Buffer === 'function' && Buffer.isBuffer(value))) {
                value = omitUndefinedProperties(_.pick(new messy.HttpResponse(value), messy.HttpResponse.propertyNames));
            }
                        ...
                        diff: function (output, diff, inspect, equal) {
                            output.block(function (output) {
                                ...
                                if (promiseByKey.statusLine.isRejected()) {
                                    ...
                                    diffResult = statusLineError.getDiff();
                                    if (diffResult) {
                                        output.append(diffResult.diff);
                                    } else {
                                        output.annotationBlock(statusLineError.getErrorMessage());
                                    }
                                } else if (promiseByKey.statusLineSpec.isRejected()) {
                                    var statusLineSpecError = promiseByKey.statusLineSpec.reason();
                                    diffResult = statusLineSpecError.getDiff();
                                    if (diffResult) {
                                        output.append(diffResult.diff);
                                    } else {
                                        output.annotationBlock(statusLineSpecError.getErrorMessage());
                                    }
                                } else {
        ...
        .addAssertion('messyHttpExchange', 'to [exhaustively] satisfy', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            }
            var flags = this.flags,
                        ...
                        diff: function (output, diff, inspect, equal) {
                            output.block(function (output) {
                                ...
                                if (promiseByKey.request.isRejected()) {
                                    ...
                                    diffResult = requestError.getDiff();
                                    if (diffResult) {
                                        output.append(diffResult.diff);
                                    } else {
                                        output.annotationBlock(requestError.getErrorMessage());
                                    }
                                ...
                                if (promiseByKey.response.isRejected()) {
                                    ...
                                    diffResult = responseError.getDiff();
                                    if (diffResult) {
                                        output.append(diffResult.diff);
                                    } else {
                                        output.annotationBlock(responseError.getErrorMessage());
                                    }
        ...
        .addAssertion('messyHttpConversation', '[not] to have number of exchanges', function (expect, subject, value) {
            expect(subject.exchanges, '[not] to have length', value);
        })
        .addAssertion('messyHttpConversation', 'to [exhaustively] satisfy', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string' || (typeof Buffer === 'function' && Buffer.isBuffer(value))) {
                            ...
                            output.block(function (output) {
                                for (var i = 0 ; i < Math.max(subject.exchanges.length, numValueExchanges) ; i += 1) {
                                    ...
                                    var exchangeSatisfyError = promiseByKey[i] && promiseByKey[i].isRejected() && promiseByKey[i].reason(),
                                        exchangeSatisfyDiff = exchangeSatisfyError && exchangeSatisfyError.getDiff();
