        ...
        try {
            if (isRegExp(value)) {
                ...
                expect(expectedHeaderNames, 'to be an array whose items satisfy', function (expectedHeaderName) {
                    ...
                    if (expectedValues.length === 1 && typeof expectedValues[0] === 'undefined') {
                        expect(subject.has(expectedHeaderName), 'to be falsy');
                    } else if (exhaustively) {
                        expect(subject.getAll(expectedHeaderName).sort(), 'to equal', expectedValues.sort().filter(function (expectedValue) {
                            return typeof expectedValue !== 'undefined';
                        }));
                    } else {
