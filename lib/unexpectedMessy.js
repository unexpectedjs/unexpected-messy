...
module.exports = {
    ...
    installInto: function unexpectedMessy(expect) {
        ...
        expect.addAssertion('<messyMessage> to [exhaustively] satisfy <any>', function (expect, subject, value) {
                ...
                if (!equal(actual, expected)) {
                    ...
                    output.sp().annotationBlock(function () {
                        this.error('should be').sp().text(shouldBeFragments.join(' '))
                            .nl(2).
                            append(diff(actual.toString(), expected.toString()).diff);
                    });
                }
                return { inline: true, diff: output };
            }
        ...
        expect.addAssertion('<messyRequestLine> to satisfy <any>', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
                        ...
                        if (messy.RequestLine.propertyNames.some(function (propertyName) {
                            ...
                            output.annotationBlock(function () {
                                this.error('should be')
                                    ...
                                    .nl(2)
                                    .append(diff(subject.toString(), _.defaults(shouldBeRequestLine.clone(), _.pick(subject, messy.RequestLine.nonComputedPropertyNames)).toString()).diff);
                            });
                        }
                        return { inline: true, diff: output };
                    }
        ...
        expect.addAssertion('<messyHttpRequest> to [exhaustively] satisfy <any>', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
        ...
        expect.addAssertion('<messyStatusLine> to satisfy <any>', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string') {
                        ...
                        if (typeof value === 'number' || typeof value === 'function') {
                            ...
                            output.annotationBlock(function () {
                                this.error('should be')
                                    ...
                                    .nl(2)
                                    .append(diff(subject.toString(), _.defaults(expectedStatusLine.clone(), _.pick(subject, messy.StatusLine.nonComputedPropertyNames)).toString()).diff);
                            });
        ...
        expect.addAssertion('<messyHttpResponse> to [exhaustively] satisfy <any>', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string' || (typeof Buffer === 'function' && Buffer.isBuffer(value))) {
        ...
        expect.addAssertion('<messyHttpExchange> to [exhaustively] satisfy <any>', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            }
            var promiseByKey = {
        ...
        expect.addAssertion('<messyHttpConversation> to [exhaustively] satisfy <any>', function (expect, subject, value) {
            if (typeof value === 'undefined') {
                value = {};
            } else if (typeof value === 'string' || (typeof Buffer === 'function' && Buffer.isBuffer(value))) {
