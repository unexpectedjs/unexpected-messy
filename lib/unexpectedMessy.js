...
module.exports = {
    ...
    installInto: function unexpectedMessy(expect) {
        ...
        expect.addStyle('messyMessageBody', function (message, depth) {
            ...
            if (typeof Buffer !== 'undefined' && Buffer.isBuffer(upgradedBody) && !contentType && /^\s*form-data(?:\s|;|$)/.test(message.headers.get('Content-Disposition')) && isAll7BitBuffer(upgradedBody)) {
        ...
        expect.exportAssertion('<messyHeaders> to [exhaustively] satisfy <regexp>', function (expect, subject, value) {
            return expect(subject.toString(), 'to match', value);
        });

        expect.exportAssertion('<messyHeaders> to [exhaustively] satisfy <any>', function (expect, subject, value) {
        ...
        expect.exportAssertion('<messyMessage> to [exhaustively] satisfy <undefined>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', {});
        });

        expect.exportAssertion('<messyMessage> to [exhaustively] satisfy <string|Buffer>', function (expect, subject, value) {
            return expect(
                subject,
                'to [exhaustively] satisfy',
                omitUndefinedProperties(_.pick(new messy.Message(value), _.difference(messy.Message.propertyNames, ['decodedBody', 'unchunkedBody', 'parts', 'rawBody'])))
            );
        });

        expect.exportAssertion('<messyMessage> to [exhaustively] satisfy <object>', function (expect, subject, value) {
            var promiseByKey = {
                ...
                body: expect.promise(function () {
                    if ('body' in value) {
                        ...
                        } else if (subject.isJson && typeof Buffer === 'function' && Buffer.isBuffer(value.body)) {
                            return expect(subject.unchunkedBody, 'to satisfy', value.body);
                        } else if (typeof value.body === 'string' && typeof Buffer !== 'undefined' && Buffer.isBuffer(subject.body) && !subject.headers.get('Content-Type') && /^\s*form-data(?:\s|;|$)/.test(subject.headers.get('Content-Disposition')) && isAll7BitBuffer(subject.body)) {
                            return expect(subject.unchunkedBody.toString('utf-8'), 'to satisfy', value.body);
                        } else {
                ...
                if (typeof value.parts !== 'undefined' && typeof value.parts !== 'function') {
                    Object.keys(value.parts).forEach(function (key) {
                        promiseByKey[key] = expect.promise(function () {
                            return expect(subject.parts && subject.parts[key], 'to satisfy', value.parts[key]);
                        });
                    });
                }
                            ...
                            output.block(function (output) {
                                ...
                                if (typeof value.parts !== 'undefined') {
                                    ...
                                    if (Array.isArray(value.parts)) {
                                        if (!subject.parts || value.parts.length !== subject.parts.length) {
                                            output.nl().annotationBlock(function () {
        ...
        expect.exportAssertion('<messyMessage> to have raw body satisfying <any>', function (expect, subject, value) {
            expect(subject.rawBody, 'to satisfy', value);
        });

        expect.exportAssertion('<messyMessage> to have unchunked body satisfying <any>', function (expect, subject, value) {
            expect(subject.unchunkedBody, 'to satisfy', value);
        });

        expect.exportAssertion('<messyMessage> to have file name satisfying <any>', function (expect, subject, value) {
            expect(subject.fileName, 'to satisfy', value);
        });

        expect.exportAssertion('<messyMessage> to have text satisfying <any>', function (expect, subject, value) {
            expect(subject.toString(), 'to satisfy', value);
        });

        expect.exportAssertion('<messyMessage> [not] to be a multipart message', function (expect, subject) {
            expect(subject.isMultipart, '[not] to be truthy');
        });

        expect.exportAssertion('<messyMessage> [not] to have number of parts <number>', function (expect, subject, value) {
        ...
        expect.exportType({
            ...
            diff: function (actual, expected, output, diff, inspect, equal) {
                ...
                if (!equal(actual, expected)) {
                    ...
                    if (error.size().width + output.size().width + 1 > output.preferredWidth) {
                        output.nl();
                    } else {
                        output.sp();
                    }
        ...
        expect.exportAssertion('<messyRequestLine> to satisfy <undefined>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', {});
        });

        expect.exportAssertion('<messyRequestLine> to satisfy <string>', function (expect, subject, value) {
        ...
        expect.exportAssertion('<messyRequestLine> to satisfy <object>', function (expect, subject, value) {
            var promiseByKey = {};
            Object.keys(value).forEach(function (key) {
                promiseByKey[key] = expect.promise(function () {
                    if (key === 'query') {
                        var queryStr = subject.query;
                        if (value.query && (typeof value.query === 'object' || typeof value.query === 'function')) {
                        ...
                        diff: function (output, diff, inspect, equal) {
                            ...
                            var error = output.clone().annotationBlock(function () {
                                if (messy.RequestLine.propertyNames.some(function (propertyName) {
                                    ...
                                    return type === 'function' || type === 'object' || isRegExp(value[propertyName]);
                                })) {
                                    ...
                                    Object.keys(promiseByKey).forEach(function (propertyName) {
                                        ...
                                        var error = promise && promise.isRejected() && promise.reason();
                                        if (error) {
                                            this.error(propertyName).sp();
                                            var type = typeof value[propertyName];
                                            if (type === 'function' || type === 'object' || isRegExp(value[propertyName])) {
                                                this.error('should satisfy').sp().appendInspected(value[propertyName]);
        ...
        expect.exportAssertion('<messyHttpRequest> to [exhaustively] satisfy <undefined>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', {});
        });

        expect.exportAssertion('<messyHttpRequest> to [exhaustively] satisfy <string|Buffer>', function (expect, subject, value) {
            return expect(
                subject,
                'to [exhaustively] satisfy',
                omitUndefinedProperties(_.pick(new messy.HttpRequest(value), _.difference(messy.HttpRequest.propertyNames, ['decodedBody', 'unchunkedBody', 'parts', 'rawBody'])))
            );
        });

        expect.exportAssertion('<messyHttpRequest> to [exhaustively] satisfy <object>', function (expect, subject, value) {
        ...
        expect.exportAssertion('<messyStatusLine> to [exhaustively] satisfy <undefined>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', {});
        });

        expect.exportAssertion('<messyStatusLine> to [exhaustively] satisfy <string>', function (expect, subject, value) {
        ...
        expect.exportAssertion('<messyStatusLine> to [exhaustively] satisfy <number>', function (expect, subject, value) {
        ...
        expect.exportAssertion('<messyStatusLine> to [exhaustively] satisfy <function|object>', function (expect, subject, value) {
            return expect.withError(function () {
                ...
                expect.fail({
                    diff: function (output, diff, inspect, equal) {
                        ...
                        var error = output.clone();
                        if (typeof value === 'number' || typeof value === 'function') {
                            if (value._expectIt) {
                                var comparison = err.getDiff(output);
                                if (comparison) {
                                    error.annotationBlock(comparison);
                                }
                            } else {
                                error.annotationBlock(function () {
        ...
        expect.exportAssertion('<messyHttpResponse> to [exhaustively] satisfy <undefined>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', {});
        });

        expect.exportAssertion('<messyHttpResponse> to [exhaustively] satisfy <string|Buffer>', function (expect, subject, value) {
            return expect(
                subject,
                'to [exhaustively] satisfy',
                omitUndefinedProperties(_.pick(new messy.HttpResponse(value), _.difference(messy.HttpResponse.propertyNames, ['decodedBody', 'unchunkedBody', 'parts', 'rawBody'])))
            );
        });

        expect.exportAssertion('<messyHttpResponse> to [exhaustively] satisfy <number>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', { statusLine: value });
        });

        expect.exportAssertion('<messyHttpResponse> to [exhaustively] satisfy <object>', function (expect, subject, value) {
        ...
        expect.exportAssertion('<messyHttpExchange> to [exhaustively] satisfy <undefined>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', {});
        });

        expect.exportAssertion('<messyHttpExchange> to [exhaustively] satisfy <object>', function (expect, subject, value) {
            var promiseByKey = {
        ...
        expect.exportAssertion('<messyHttpConversation> [not] to have number of exchanges <number>', function (expect, subject, value) {
            expect(subject.exchanges, '[not] to have length', value);
        });

        expect.exportAssertion('<messyHttpConversation> to [exhaustively] satisfy <string|Buffer>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', new messy.HttpConversation(value));
        });

        expect.exportAssertion('<messyHttpConversation> to [exhaustively] satisfy <undefined>', function (expect, subject, value) {
            return expect(subject, 'to [exhaustively] satisfy', {});
        });

        expect.exportAssertion('<messyHttpConversation> to [exhaustively] satisfy <object>', function (expect, subject, value) {
